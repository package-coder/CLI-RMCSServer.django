...
class AFType(models.Model):
    form_number = models.IntegerField(primary_key=True, editable=False, unique=True)
...
class AFTransactionHistory(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    issued_to = models.ForeignKey(PersonTransactionRecord, on_delete=models.RESTRICT)
    operator = models.ForeignKey(User, on_delete=models.RESTRICT)
    transaction_type = models.ForeignKey(AFTransactionType, on_delete=models.RESTRICT, null=True)
    request_history = models.ForeignKey(AFRequestHistory, on_delete=models.RESTRICT, null=True)
    prefix = models.ForeignKey(AFPrefix, on_delete=models.RESTRICT, default='PREFIX_TRANSACTION')
    status = models.ForeignKey(AFTransactionStatus, on_delete=models.RESTRICT, default='STATUS_COMPLETED')
    control_number = models.IntegerField(default=1)
...
class AFItem(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    af_type = models.ForeignKey(AFType, on_delete=models.RESTRICT)
    state = models.ForeignKey(AFState, on_delete=models.RESTRICT, default='STATE_OPEN')
    ownership = models.ForeignKey(PersonTransactionRecord, on_delete=models.RESTRICT, null=True)
